...
/* Default Missions */ const DEFAULT_MISSIONS: Mission[] = [
  {
    id: "jantar_casa",
...
export const MissoesTab: React.FC<MissoesTabProps> = ({
  ...
  /* Add new mission state */ const [showAddForm, setShowAddForm] =
    useState(false);
  ...
  /* Merge all missions */ const allMissions = useMemo(() => {
    const missions: Mission[] = [];
    if (battleChallenges.length > 0) {
      missions.push(
        ...DEFAULT_MISSIONS.filter((m) => m.category === "economia"),
      );
  ...
  /* ======================================== */ /* Filtered missions */ const filteredMissions =
    useMemo(() => {
  ...
  /* ======================================== */ /* Streak calculation */ const streaks =
    useMemo(() => {
      ...
      allMissions.forEach((mission) => {
        ...
        /* Total count */ const count = missionDeposits.length;
        /* Streak: consecutive days with this mission */ const dates =
          Array.from(
            new Set(
              missionDeposits
                .map((d) => {
  ...
  /* ======================================== */ /* Complete a mission */ const handleImageUpload =
  ...
  const handleComplete = async () => {
    ...
    const finalAmount =
      selectedMission.reward > 0 ? selectedMission.reward : parsedAmount;
  ...
  /* ======================================== */ /* Edit mission */ const handleEditClick =
    (mission: Mission) => {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      /* Update in the appropriate Firestore collection */ const isDesafio =
  ...
  /* ======================================== */ /* Delete mission */ const handleDeleteMission =
  ...
  /* ======================================== */ /* Add new mission */ const handleAddMission =
    async () => {
      ...
      try {
        ...
        addToast(
          "Boa Missão!",
          "Nova tarefa adicionada ao painel. Bora fazer!",
          "success",
        );
        setNewTitle("");
        setNewDesc("");
  ...
  /* ======================================== */ /* Category badge helper */ const getCategoryBadge =
    (category: string) => {
      switch (category) {
        case "economia":
          return {
  ...
  /* ======================================== */ /* Render */ return (
    <div className="pb-24 pt-6 px-6 max-w-md mx-auto space-y-6">
        ...
        {FILTERS.map((filter) => (
          ...
          >
            ...
            >
              ...
              {filter.id === "todas"
                ? allMissions.length
                : allMissions.filter((m) => m.category === filter.id)
                    .length}{" "}
            </span>{" "}
          </button>
        ))}{" "}
        ...
        {filteredMissions.length === 0 ? (
          ...
          filteredMissions.map((mission) => {
            ...
            return (
              <div
                key={mission.id}
                className="bg-cookbook-bg backdrop-blur-2xl border border-cookbook-border rounded-3xl p-5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-all hover:shadow-[0_10px_40px_rgb(0,0,0,0.08)] group relative overflow-hidden"
              >
                {" "}
                {/* Top accent line based on category */}{" "}
                <div
                  className={`absolute top-0 left-0 w-full h-1 ${mission.category === "economia" ? "bg-emerald-400" : mission.category === "desafio" ? "bg-amber-400" : "bg-violet-400"}`}
                />{" "}
      ...
      {!showAddForm ? (
        ...
        <div className="bg-cookbook-bg backdrop-blur-2xl border border-cookbook-border rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] space-y-4 animate-modal-enter">
          {" "}
          <div className="flex items-center justify-between">
            {" "}
            <h4 className="font-serif text-lg text-cookbook-text font-medium">
              {" "}
              Criar Missão{" "}
            </h4>{" "}
            <button
          ...
          <div className="flex gap-2">
            ...
            <input
              ...
              className="flex-1 bg-cookbook-bg/90 backdrop-blur-md border border-cookbook-border rounded-xl px-4 py-3 font-serif text-sm text-cookbook-text focus:outline-none focus:border-cookbook-primary transition-colors shadow-sm"
            />{" "}
          </div>{" "}
          {newCategory === "desafio" && (
            <>
              ...
              <input
                ...
                className="w-full bg-cookbook-bg/90 backdrop-blur-md border border-cookbook-border rounded-xl px-4 py-3 font-sans text-xs text-cookbook-text focus:outline-none focus:border-cookbook-primary transition-colors shadow-sm"
              />{" "}
      ...
      {selectedMission && (
        ...
        >
          ...
          >
            ...
            {/* Colored accent */}{" "}
            <div
              className={`absolute top-0 left-0 w-full h-1 rounded-t-2xl ${selectedMission.category === "economia" ? "bg-gradient-to-r from-emerald-400 to-emerald-300" : selectedMission.category === "desafio" ? "bg-gradient-to-r from-amber-400 to-amber-300" : "bg-gradient-to-r from-violet-400 to-violet-300"}`}
            />{" "}
            <button
              onClick={() => {
                setSelectedMission(null);
            ...
            <div className="space-y-4">
              ...
              >
                ...
                {isSubmitting
                  ? "Guardando..."
                  : selectedMission.reward > 0
                    ? `Guardar R$ ${selectedMission.reward}`
                    : "Guardar no Pote"}{" "}
              </button>{" "}
            </div>{" "}
          </div>{" "}
