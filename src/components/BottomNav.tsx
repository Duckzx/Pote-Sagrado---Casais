...
// Custom SVG matching the theme format, designed to perfectly resemble the animated Safe Pot
const SacredPotIcon = ({ size = 24, strokeWidth = 2, className = "" }) => (
  <svg 
...
export const BottomNav: React.FC<BottomNavProps> = ({ activeTab, setActiveTab }) => {
  // Reduced to 5 focused items, removing Extrato since it mixes with Home's Diário de Bordo. 
  // Focus is now balanced around a prominent center.
  const tabsLeft = [
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive ? "text-cookbook-primary" : "hover:text-cookbook-text active:scale-95"
                )}
              >
                {isActive && (
                  <motion.div
        ...
        <div className="w-[20%] flex justify-center relative">
          <button
            ...
            className={cn(
              ...
              activeTab === 'home' ? 'bg-cookbook-primary scale-105' : 'bg-cookbook-primary/90 hover:bg-cookbook-primary hover:scale-105 active:scale-95'
            )}
          >
            <SacredPotIcon 
              size={24} 
              strokeWidth={activeTab === 'home' ? 2.5 : 2} 
              className={cn("transition-colors", activeTab === 'home' ? 'text-cookbook-gold' : 'text-white')} 
            />
          </button>
        </div>
