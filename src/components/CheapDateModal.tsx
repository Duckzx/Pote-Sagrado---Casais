...
export const CheapDateModal: React.FC<CheapDateModalProps> = ({
  onClose,
  currentUser,
}) => {
  ...
  /* Form state */ const [newTitle, setNewTitle] = useState("");
  const [newIdea, setNewIdea] = useState("");
  const [newCost, setNewCost] = useState("");
  const [newIconId, setNewIconId] = useState("heart");
  /* Load custom dates on mount */ useEffect(() => {
  ...
  const handleGenerate = () => {
    ...
    setResult(null);
    setTimeout(() => {
      /* Base ideas */ const ideas = [...(DATE_IDEAS[selectedTier] || [])];
      /* Incorporate custom ideas for the selected tier */ const customForTier =
        customDates
          .filter((d) => d.tier === selectedTier)
          .map((c) => {
            const iconMatch = ICONS.find((i) => i.id === c.iconId)?.icon || (
              <Star size={24} />
            );
            return {
              title: c.title,
              idea: c.idea,
              cost: c.cost,
              icon: React.cloneElement(iconMatch as React.ReactElement<any>, {
                size: 24,
              }),
            };
          });
      ...
      const randomIdea =
        allIdeas.length > 0
          ...
          : {
              title: "Nada definido",
              idea: "Não há ideias para esse limite ainda.",
              cost: "-",
              icon: <Star size={24} />,
            };
  ...
  /* If we are showing the add form */ if (isAdding) {
    return (
      <div
      ...
      >
        {" "}
        <div
          className="bg-cookbook-bg shadow-xl border border-cookbook-border rounded-3xl w-full max-w-sm p-6 shadow-2xl relative overflow-hidden animate-modal-enter h-[80vh] flex flex-col"
        ...
        >
          ...
          <div className="flex items-center justify-between mb-4 border-b border-cookbook-border/30 pb-2">
            {" "}
            <h3 className="font-serif text-lg text-cookbook-text font-medium">
              {" "}
              Adicionar Encontro{" "}
            </h3>{" "}
            <button
          ...
          <div className="flex-1 overflow-y-auto space-y-4 px-1 custom-scrollbar">
            ...
            <div>
              ...
              <input
                ...
                maxLength={40}
                className="w-full bg-cookbook-bg/90 backdrop-blur-md border border-cookbook-border rounded-2xl p-3 font-serif text-sm text-cookbook-text focus:outline-none focus:border-cookbook-primary transition-colors"
              />{" "}
            </div>{" "}
            <div>
              {" "}
              ...
              <textarea
                ...
                maxLength={150}
                className="w-full bg-cookbook-bg/90 backdrop-blur-md border border-cookbook-border rounded-2xl p-3 font-serif text-sm text-cookbook-text h-20 resize-none focus:outline-none focus:border-cookbook-primary transition-colors"
              />{" "}
            </div>{" "}
    ...
    >
      ...
      >
        ...
        <div className="mt-2 text-center">
          {" "}
          <div
            className="w-14 h-14 bg-cookbook-primary/10 rounded-full flex items-center justify-center mx-auto mb-4 border border-cookbook-primary/20 shadow-sm relative group cursor-pointer"
          ...
          <h3 className="font-serif text-xl text-cookbook-text mb-2">
            {" "}
            Roleta de Encontros{" "}
          </h3>{" "}
