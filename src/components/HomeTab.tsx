...
const MilestoneTracker = ({ totalSaved, goalAmount, onRewardClick }: { totalSaved: number, goalAmount: number, onRewardClick: () => void }) => {
  ...
  return (
    <div className="bg-white/40 dark:bg-black/10 backdrop-blur-2xl border border-amber-300/40 rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] animate-fade-in -mt-4 relative z-10 text-center relative overflow-hidden">
      ...
      <h4 className="font-serif italic text-xl text-cookbook-text mb-1">Parabéns! Pote chegou a {activeMilestone.label}</h4>
...
export const HomeTab: React.FC<HomeTabProps> = ({ currentUser, destination, origin, goalAmount, totalSaved, deposits, achievements = [], targetDate, addToast }) => {
  ...
  return (
    <div className="space-y-10 pb-24 pt-6 px-6 max-w-md mx-auto relative">
      ...
      <div className="text-center bg-white/40 dark:bg-black/10 backdrop-blur-md border border-white/40 dark:border-white/5 rounded-2xl px-5 py-3 -mt-4 shadow-sm">
        <span className="text-lg mr-1.5">{dailyQuote.emoji}</span>
        <span className="font-serif italic text-[13px] text-cookbook-text/80">{dailyQuote.text}</span>
      </div>
        ...
        >
          ...
          <div className="flex items-center space-x-4 relative z-10">
            ...
            <div className="text-left">
              <p className="font-serif italic text-base text-white">Nosso Momento Wrapped</p>
              <p className="font-sans text-[10px] uppercase tracking-widest text-white/80 font-medium">Resumo do Casal</p>
            </div>
          </div>
          <ArrowRight size={18} className="text-white/70 relative z-10" strokeWidth={2} />
        </button>
      </div>
            ...
            <div className="space-y-4 mb-6">
              <div className="relative">
                <input
                  ...
                  placeholder="R$ 0,00"
                  className="w-full bg-white/40 dark:bg-black/10 backdrop-blur-md border border-white/40 dark:border-white/5 rounded-2xl py-3 pr-4 font-serif text-2xl text-center text-cookbook-text focus:outline-none focus:border-cookbook-primary transition-colors"
                />
              </div>
              <input
                type="text"
        ...
        >
          ...
          >
            ...
            <div className="flex space-x-3 pt-4">
              <button 
                onClick={() => setShowBreakConfirm(false)}
                className="flex-1 py-3 bg-white/10 hover:bg-white/20 transition-colors text-white font-sans text-[10px] uppercase tracking-widest font-bold rounded-2xl"
              >
                Cancelar
              </button>
              <button 
      ...
      {showQuickDeposit && createPortal(
        <div
          className="fixed inset-0 z-[60] flex items-end justify-center bg-white/40 dark:bg-black/40 backdrop-blur-md animate-modal-backdrop"
          onClick={() => {
            setShowQuickDeposit(false);
            setQuickImage(null);
          }}
          ...
          >
            ...
            <div className="flex gap-2 mb-4">
              <button
                ...
                className={`flex-1 py-3 rounded-2xl font-sans text-[10px] uppercase tracking-widest font-bold border transition-all ${
                  quickType === 'income' ? 'bg-emerald-500 text-white border-emerald-500 shadow-sm' : 'bg-white/40 dark:bg-black/10 backdrop-blur-md border border-white/40 dark:border-white/5 text-cookbook-text/50'
                }`}
              >
                ↑ Entrada
              </button>
              <button
                ...
                className={`flex-1 py-3 rounded-2xl font-sans text-[10px] uppercase tracking-widest font-bold border transition-all ${
                  quickType === 'expense' ? 'bg-red-500 text-white border-red-500 shadow-sm' : 'bg-white/40 dark:bg-black/10 backdrop-blur-md border border-white/40 dark:border-white/5 text-cookbook-text/50'
                }`}
              >
                ↓ Saída
              </button>
            ...
            <div className="space-y-3 mb-5">
              <div className="relative">
                <input
                  ...
                  placeholder="R$ 0,00"
                  className="w-full bg-white/40 dark:bg-black/10 backdrop-blur-md border border-white/40 dark:border-white/5 rounded-2xl py-4 pr-4 font-serif text-3xl text-cookbook-text text-center focus:outline-none focus:border-cookbook-primary transition-colors"
                  autoFocus
                />
              </div>
              <input
              ...
              {quickType === 'income' && (
                <div className="mt-4">
                  {quickImage ? (
                    <div className="relative w-full h-32 rounded-2xl overflow-hidden border border-white/40 dark:border-white/5">
                    ...
                    <label className="flex flex-col items-center justify-center w-full h-20 border-2 border-dashed border-white/40 dark:border-white/5 rounded-2xl cursor-pointer bg-white/40 dark:bg-black/10 backdrop-blur-md hover:bg-cookbook-primary/10 transition-colors">
            ...
            <button
              ...
              className={`w-full text-white font-sans text-[10px] uppercase tracking-widest py-4 rounded-2xl font-bold shadow-lg disabled:opacity-50 transition-all active:scale-[0.98] ${
