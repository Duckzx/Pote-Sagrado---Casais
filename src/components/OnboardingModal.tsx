...
export const OnboardingModal: React.FC<OnboardingModalProps> = ({ onComplete }) => {
  ...
  const steps = [
    {
      ...
      description: 'O lugar onde vocês guardam dinheiro juntos para realizar a viagem dos sonhos. Cada centavo conta!'
    },
    {
      icon: <Target size={48} className="text-cookbook-primary mx-auto mb-4" />,
      title: 'Bingo de Atitudes',
      description: 'Pequenas escolhas viram grandes viagens. Evitou o delivery hoje? Fez o café em casa? Registre no Bingo e guarde o valor no Pote!'
    },
    {
      icon: <Trophy size={48} className="text-cookbook-primary mx-auto mb-4" />,
      title: 'Disputa Saudável',
      description: 'Acompanhem quem está guardando mais dinheiro no mês. Que tal o perdedor pagar um lanche no final de semana?'
    }
  ];
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-cookbook-bg/90 backdrop-blur-sm">
      <div className="bg-white border border-cookbook-border rounded w-full max-w-sm p-8 shadow-2xl relative overflow-hidden text-center">
        ...
        <div className="min-h-[220px] flex flex-col justify-center">
          {steps[step].icon}
          <h2 className="font-serif text-2xl text-cookbook-text mb-3">{steps[step].title}</h2>
        ...
        >
          <span>{step === steps.length - 1 ? 'Começar a Guardar' : 'Próximo'}</span>
