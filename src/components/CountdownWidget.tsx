...
export const CountdownWidget: React.FC<CountdownWidgetProps> = ({
  ...
  useEffect(() => {
    ...
    /* update every minute */ return () => clearInterval(interval);
  }, []);
  ...
  /* Progress: from when config was set to target. We show percentage of time elapsed assuming 365 day max window */ const maxDaysWindow = 365;
  ...
  if (isPast(date) && !isToday(date)) {
    return (
      <div className="bg-gradient-to-r from-emerald-50 to-emerald-100 border border-emerald-200/60 rounded-xl p-4 text-center">
        ...
        <p className="font-serif italic text-sm text-emerald-700">
          {" "}
          A viagem já chegou!{" "}
        </p>{" "}
        <p className="font-sans text-[9px] uppercase tracking-widest text-emerald-500 font-bold mt-1">
          {" "}
          Boa viagem, aproveitem cada momento!{" "}
        </p>{" "}
      </div>
    );
  }
  if (isToday(date)) {
    return (
      <div className="bg-gradient-to-r from-amber-50 to-yellow-50 border border-amber-200/60 rounded-xl p-4 text-center animate-pulse">
        ...
        <p className="font-serif italic text-sm text-amber-800">
          {" "}
          Hoje é o grande dia!{" "}
        </p>{" "}
        <p className="font-sans text-[9px] uppercase tracking-widest text-amber-600 font-bold mt-1">
          {" "}
          É HOJE! Boa viagem!{" "}
        </p>{" "}
      </div>
  ...
  if (isTomorrow(date)) {
    return (
      <div className="bg-gradient-to-r from-cookbook-mural to-amber-50 border border-cookbook-gold/30 rounded-xl p-4 text-center">
        {" "}
        <span className="text-2xl block mb-1">🧳</span>{" "}
        <p className="font-serif italic text-sm text-cookbook-text">
          {" "}
          Amanhã é o dia!{" "}
        </p>{" "}
        <p className="font-sans text-[9px] uppercase tracking-widest text-cookbook-gold font-bold mt-1">
          {" "}
          Arrume as malas!{" "}
        </p>{" "}
      </div>
