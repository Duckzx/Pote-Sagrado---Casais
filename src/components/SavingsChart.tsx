...
export const SavingsChart: React.FC<SavingsChartProps> = ({ deposits, goalAmount }) => {
  ...
  const dataPoints = useMemo(() => {
    ...
    // Add starting zero point 1 day before first deposit
    const firstDate = sorted[0].createdAt.toDate();
    const startDate = new Date(firstDate);
    startDate.setDate(startDate.getDate() - 1);
    points.push({ date: startDate, value: 0, label: startDate.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' }) });
  ...
  useEffect(() => {
    ...
    const primaryColor = styles.getPropertyValue('--theme-primary').trim() || '#8E7F6D';
    const textColor = styles.getPropertyValue('--theme-text').trim() || '#2C2A26';
    const borderColor = styles.getPropertyValue('--theme-border').trim() || '#E8E4D9';
    const goldColor = styles.getPropertyValue('--theme-gold').trim() || '#C5A059';
    ...
    // Map functions
    const mapX = (date: Date) => padding.left + ((date.getTime() - timeStart) / timeRange) * chartW;
    const mapY = (val: number) => padding.top + (1 - (val - minVal) / valRange) * chartH;
    ...
    ctx.fillText(dataPoints[0].label, mapX(dataPoints[0].date), h - padding.bottom + 8);
    ctx.fillText(dataPoints[dataPoints.length - 1].label, mapX(dataPoints[dataPoints.length - 1].date), h - padding.bottom + 8);
    ...
    // Data points (dots)
    dataPoints.forEach((p, i) => {
      if (i === 0) return; // skip the zero anchor
      const x = mapX(p.date);
      const y = mapY(p.value);
  ...
  // Touch/mouse interaction for tooltip
  const handleInteraction = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    setTooltip({
      ...
      y: 20,
      value: closest.value,
      date: closest.date.toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' })
    });
  };
    ...
    <div className="bg-white border border-cookbook-border rounded-xl p-4 shadow-sm space-y-2">
      ...
      >
        ...
        {tooltip && (
          ...
          >
            <div className="font-serif text-xs font-medium">{formatCurrency(tooltip.value)}</div>
            <div className="font-sans text-[7px] uppercase tracking-widest opacity-60">{tooltip.date}</div>
          </div>
        )}
      </div>
    </div>
